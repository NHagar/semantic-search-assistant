...
import { corpusName, selectedLLM, setError, setLoading } from './stores.js';
  ...
  try {
    ...
    const uploadedDocs = (uploadedResponse.documents || [])
      .filter(doc => {
        ...
        return !embeddedNames.has(doc.filename)
          && !embeddedNames.has(txtVersion);
      })
      .map(doc => ({
        file: null,
        name: doc.filename,
...
function normalizeFilename(filename) {
  ...
  if (
    parts.length >= 2
    && (parts[parts.length - 1].toLowerCase() === 'pdf'
      || parts[parts.length - 1].toLowerCase() === 'txt')
  ) {
...
async function extractTextFromPDF(file) {
  try {
    ...
    let res = await apiService.extractRawText(file, llm, corpus);
...
async function embedAndProceed() {
  ...
  if (!corpus) {
    setError(
      'No project selected. Please go back and select or create a project.',
    );
    return;
  }
  ...
  try {
    ...
    const modifiedDocuments = documents.filter(doc =>
      doc.isExisting && doc.isModified
    );
    const unchangedDocuments = documents.filter(doc =>
      doc.isExisting && !doc.isModified
    );
    const serverUploadedDocuments = newDocuments.filter(doc =>
      doc.isUploadedFromServer
    );
    ...
    const browserUploadedDocuments = newDocuments.filter(doc =>
      !doc.isUploadedFromServer
    );
    ...
    if (filesToUpload.length > 0) {
      progressOperations.push({
        label: `Uploading ${filesToUpload.length} new document${
    ...
    if (documentsToSave.length > 0) {
      progressOperations.push({
        label: `Saving updated text for ${documentsToSave.length} document${
          documentsToSave.length !== 1 ? 's' : ''
        }...`,
    ...
    if (documentsToSave.length > 0 || serverUploadedDocuments.length > 0) {
      progressOperations.push({
        ...
        weight: Math.max(
          documentsToSave.length + serverUploadedDocuments.length,
          1,
        ),
        action: async () => {
    ...
    const totalWeight = progressOperations.reduce(
    ...
    if (totalWeight === 0) {
      ...
      embeddingProgressText =
        'No document changes detected. Skipping embedding.';
    } else {
      ...
      <div class="drop-zone-content">
        ...
        {#if extractingCount > 0}
          <div class="extraction-progress">
            ...
            <span>Extracting text from {extractingCount} document{
                extractingCount !== 1 ? 's' : ''
              }...</span>
          </div>
        {/if}
  ...
  {:else}
          ...
          {#each documents as doc, index}
            ...
            >
              ...
              {#if !doc.extracting}
                <button
                  class="remove-btn"
                  on:click|stopPropagation={() => removeDocument(index)}
        ...
        {#if documents[selectedDocIndex]}
          <div class="editor-header">
            ...
            {#if documents[selectedDocIndex].file}
              <span class="file-size">{
                formatFileSize(documents[selectedDocIndex].file.size)
              }</span>
            {:else if documents[selectedDocIndex].size}
              <span class="file-size">{
                formatFileSize(documents[selectedDocIndex].size)
              }</span>
            {/if}
          </div>
          ...
          {:else if documents[selectedDocIndex].isUploadedFromServer}
            <div class="info-message">
              ...
              <div>
                <p>
                  This document was previously uploaded and is waiting to be
                  embedded.
                </p>
                <p class="info-subtext">
                  Run "Embed & Continue" to process it, or re-upload the PDF to
                  edit the extracted text before embedding.
                </p>
                {#if documents[selectedDocIndex].uploadedAt}
                  <p class="info-meta">
                    Uploaded on {
                      formatUploadDate(documents[selectedDocIndex].uploadedAt)
                    }
                  </p>
                {/if}
              </div>
          ...
          {:else}
            ...
            <div class="editor-footer">
              <span class="char-count">{editedText.length.toLocaleString()}
                characters</span>
            </div>
          {/if}
        {/if}
    ...
    <div class="action-buttons">
      <button
        ...
        disabled={embedding || extractingCount > 0}
      >
        <svg
          width="16"
          height="16"
      ...
      <button
        class="embed-btn"
        on:click={embedAndProceed}
        disabled={embedding || extractingCount > 0 || documents.length === 0}
      >
        {#if embedding}
          <div class="mini-spinner"></div>
          Embedding Documents...
    ...
    {#if embeddingProgressVisible}
      <div class="embedding-progress">
        <div class="embedding-progress-header">
          ...
          <span class="embedding-progress-value">{
        ...
        <div class="embedding-progress-bar">
          <div
            ...
            style={`width: ${Math.max(0, Math.min(100, embeddingProgress))}%`}
          >
          </div>
        </div>
      </div>
